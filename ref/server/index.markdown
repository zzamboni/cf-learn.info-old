---
layout: cf-learn
title: Learning CFEngine 3
pageurl: /ref/server
posttitle: Reference disambiguation for 'server'
navsection: none
---

The term 'server' could refer to several sections in the documentation. Please choose one below, or
[search the CFEngine documentation for 'server'](http://cfengine.com/docs/latest/search.html?q=server).

- [Client-server example](http://cfengine.com/docs/latest/examples-example-snippets-network.html#client-server-example)
- [Set up a PXE boot server](http://cfengine.com/docs/latest/examples-example-snippets-network.html#set-up-a-pxe-boot-server)
- [Updating from a central policy server](http://cfengine.com/docs/latest/examples-example-snippets-promise-patterns-example_updating_from_central_hub.html#updating-from-a-central-policy-server)
- [Set up a web server](http://cfengine.com/docs/latest/examples-example-snippets-software-adminstration.html#set-up-a-web-server)
- [Web server modules](http://cfengine.com/docs/latest/examples-example-snippets-software-adminstration.html#web-server-modules)
- [What is the system user for the CFEngine dedicated PostgreSQL database and Apache server \#\#\#\#](http://cfengine.com/docs/latest/guide-faq.html#what-is-the-system-user-for-the-cfengine-dedicated-postgresql-database-and-apache-server-####)
- [My policy server has changed its IP address and new bootstraps don't work\!](http://cfengine.com/docs/latest/guide-faq.html#my-policy-server-has-changed-its-ip-address-and-new-bootstraps-don-t-work!)
- [Policy Server \#\#\#\#](http://cfengine.com/docs/latest/guide-glossary.html#policy-server-####)
- [Server \#\#\#\#](http://cfengine.com/docs/latest/guide-glossary.html#server-####)
- [2. Install CFEngine on a Policy Server](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-community.html#2-install-cfengine-on-a-policy-server)
- [3. Bootstrap the Policy Server](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-community.html#3-bootstrap-the-policy-server)
- [5. Bootstrap the Host to the Policy Server](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-community.html#5-bootstrap-the-host-to-the-policy-server)
- [Configure the Firewall on the Policy Server (AKA hub) \#\#\#](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise-free-aws-rhel.html#configure-the-firewall-on-the-policy-server-aka-hub-###)
- [Step 1. Download and install Enterprise on a Policy Server \#\#\#](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise-free-aws-rhel.html#step-1-download-and-install-enterprise-on-a-policy-server-###)
- [Step 2. Bootstrap the Policy Server \#\#\#](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise-free-aws-rhel.html#step-2-bootstrap-the-policy-server-###)
- [Step 4. Bootstrap the Host to the Policy Server \#\#\#](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise-free-aws-rhel.html#step-4-bootstrap-the-host-to-the-policy-server-###)
- [1. Download and install Enterprise on a Policy Server](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise-free.html#1-download-and-install-enterprise-on-a-policy-server)
- [2. Bootstrap the Policy Server](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise-free.html#2-bootstrap-the-policy-server)
- [4. Bootstrap the Host to the Policy Server](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise-free.html#4-bootstrap-the-host-to-the-policy-server)
- [Policy Server Requirements](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation-installation-enterprise.html#policy-server-requirements)
- [Server IP Address and Hostname \#\#\#](http://cfengine.com/docs/latest/guide-installation-and-configuration-general-installation.html#server-ip-address-and-hostname-###)
- [Upgrade masterfiles and Policy Server (3.6.X to 3.6.X\+1)](http://cfengine.com/docs/latest/guide-installation-and-configuration-upgrading-to-36.html#upgrade-masterfiles-and-policy-server-3-6-x-to-3-6-x+1)
- [Prepare masterfiles and the Policy Server for upgrade (3.5 to 3.6)](http://cfengine.com/docs/latest/guide-installation-and-configuration-upgrading-to-36.html#prepare-masterfiles-and-the-policy-server-for-upgrade-3-5-to-3-6)
- [Perform the upgrade of the Policy Server (3.5 to 3.6)](http://cfengine.com/docs/latest/guide-installation-and-configuration-upgrading-to-36.html#perform-the-upgrade-of-the-policy-server-3-5-to-3-6)
- [Connecting to server](http://cfengine.com/docs/latest/guide-introduction-networking.html#connecting-to-server)
- [Server normal ordering](http://cfengine.com/docs/latest/guide-language-concepts-normal-ordering.html#server-normal-ordering)
- [Enterprise Server \#\#](http://cfengine.com/docs/latest/guide-latest-release-supported-platforms.html#enterprise-server-##)
- [database_server](http://cfengine.com/docs/latest/reference-promise-types-databases.html#database_server)
- [db_server_owner](http://cfengine.com/docs/latest/reference-promise-types-databases.html#db_server_owner)
- [db_server_password](http://cfengine.com/docs/latest/reference-promise-types-databases.html#db_server_password)
- [db_server_host](http://cfengine.com/docs/latest/reference-promise-types-databases.html#db_server_host)
- [db_server_type](http://cfengine.com/docs/latest/reference-promise-types-databases.html#db_server_type)
- [db_server_connection_db](http://cfengine.com/docs/latest/reference-promise-types-databases.html#db_server_connection_db)
- [mount_server](http://cfengine.com/docs/latest/reference-promise-types-storage.html#mount_server)
