---
layout: cf-learn
title: Learning CFEngine 3
pageurl: /ref/output
posttitle: Reference disambiguation for 'output'
navsection: none
---

The term 'output' could refer to several sections in the documentation. Please choose one below, or
[search the CFEngine documentation for 'output'](http://cfengine.com/docs/3.5/search.html?q=output).

- [Output Directories](http://cfengine.com/docs/3.5/manuals-components.html#output-directories)
- [output_to_file](http://cfengine.com/docs/3.5/reference-components-cfrunagent.html#output_to_file)
- [output_directory](http://cfengine.com/docs/3.5/reference-components-cfrunagent.html#output_directory)
- [output_prefix](http://cfengine.com/docs/3.5/reference-components.html#output_prefix)
- [no_output](http://cfengine.com/docs/3.5/reference-promise-types-commands.html#no_output)
