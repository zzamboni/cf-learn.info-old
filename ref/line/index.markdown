---
layout: cf-learn
title: Learning CFEngine 3
pageurl: /ref/line
posttitle: Reference disambiguation for 'line'
navsection: none
---

The term 'line' could refer to several sections in the documentation. Please choose one below, or
[search the CFEngine documentation for 'line'](http://cfengine.com/docs/latest/search.html?q=line).

- [Warn if matching line in file](http://cfengine.com/docs/latest/examples-example-snippets-basic-file-directory.html#warn-if-matching-line-in-file)
- [Command Line Sketches](http://cfengine.com/docs/latest/guide-design-center-configure-sketches-community.html#command-line-sketches)
- [Enterprise and Community Users can Write Sketches from the Command Line](http://cfengine.com/docs/latest/guide-design-center-design-center-write-sketch-advanced.html#enterprise-and-community-users-can-write-sketches-from-the-command-line)
- [Command-Line Reports](http://cfengine.com/docs/latest/guide-reporting-command-line-reports.html#command-line-reports)
- [Command-line reporting is available to Enterprise and Community users.](http://cfengine.com/docs/latest/guide-reporting-command-line-reports.html#command-line-reporting-is-available-to-enterprise-and-community-users)
- [Command-Line Reporting](http://cfengine.com/docs/latest/guide-reporting.html#command-line-reporting)
- [Whitespace and Line Length](http://cfengine.com/docs/latest/guide-writing-and-serving-policy-policy-style.html#whitespace-and-line-length)
- [select_line_matching](http://cfengine.com/docs/latest/reference-promise-types-measurements.html#select_line_matching)
- [bundle edit_line](http://cfengine.com/docs/latest/reference-promise-types-edit_line.html#bundle-edit_line)
- [edit_line](http://cfengine.com/docs/latest/reference-promise-types-files.html#edit_line)
- [select_line_number](http://cfengine.com/docs/latest/reference-promise-types-measurements.html#select_line_number)
